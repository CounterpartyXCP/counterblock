...
class ChatFeedServerNamespace(BaseNamespace, BroadcastMixin):
    ...
    def on_command(self, command, args):
        ...
        if command == 'online':  # /online <handle>
            if not self.socket.session['is_primary_server']:
                return
            if len(args) != 1:
            ...
            if self.socket.session['banned_until'] and self.socket.session['banned_until'] >= now:
                return self.error(
                    'banned', "Your handle is still banned from chat for %s more seconds."
            ...
            online_clients[p['wallet_id']]['state'].emit(
                "emote", self.socket.session['handle'],
            ...
            if len(args) != 2:
                return self.error(
                    'invalid_args', "USAGE: /ban {handle to ban} {ban_period in sec | -1} -- " +
            ...
            if self.socket.session['is_primary_server']:  # let all users know
                self.broadcast_event(
                    "banned", self.socket.session['handle'], p['handle'], ban_period,
            ...
            if len(args) != 2:
                return self.error(
                    'invalid_args', "USAGE: /handle {oldhandle} {newhandle} -- Desc: Change a user's handle to something else")
            handle = args[0]
            new_handle = args[1]
            ...
            if len(args) != 1:
                return self.error(
                    ...
                    % (command, "Disables extended asset information from showing" if command == 'disextinfo' else "(Re)enables extended asset information"))
            asset = args[0].upper()
            ...
            return self.emit(
                'emote', None,
                "Asset '%s' extended info %s"
                % (asset, 'disabled' if command == 'disextinfo' else 'enabled'), False, True)  # isPrivate = False, viaCommand = True
        elif command == 'help':
