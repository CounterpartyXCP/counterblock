...
def serve_api(mongo_db, redis_client):
  ...
  def get_messagefeed_messages_by_index(message_indexes):
    messages = \
    util.call_jsonrpc_api("get_messages_by_index", {'message_indexes': message_indexes}, abort_on_error=True)['result']
    events = []
    for m in messages:
    ...
    for address in addresses:
      ...
      if with_last_txn_hashes:
        ...
        result['last_txns'] = txns[
                              info['unconfirmedTxApperances']:with_last_txn_hashes + info['unconfirmedTxApperances']]
  ...
  def _get_address_history(address, start_block=None, end_block=None):
    ...
    address_dict['order_matches'] = util.call_jsonrpc_api("get_order_matches",
                                                          {'filters': [
                                                            {'field': 'tx0_address', 'op': '==', 'value': address},
                                                            {'field': 'tx1_address', 'op': '==', 'value': address}, ],
                                                           'filterop': 'or',
    ...
    address_dict['bet_match_expirations'] = util.call_jsonrpc_api("get_bet_match_expirations",
                                                                  {'filters': [{'field': 'tx0_address', 'op': '==',
                                                                                'value': address},
                                                                               {'field': 'tx1_address', 'op': '==',
                                                                                'value': address}, ],
                                                                   'filterop': 'or',
                                                                   'order_by': 'block_index',
                                                                   'order_dir': 'asc',
  ...
  def get_raw_transactions(address, start_ts=None, end_ts=None, limit=500):
    ...
    txns = txns[
           0:limit]  #TODO: we can trunk before sorting. check if we can use the messages table and use sql order and limit
  ...
  def get_market_cap_history(start_ts=None, end_ts=None):
    ...
    for market_cap_as in (config.XCP, config.BTC):
      ...
      data[market_cap_as] = {}
      for e in caps:
        interval_time = int(time.mktime(
          datetime.datetime(e['_id']['year'], e['_id']['month'], e['_id']['day'], e['_id']['hour']).timetuple()) * 1000)
  ...
  def get_market_info_leaderboard(limit=100):
    ...
    assets_market_info_xcp = list(
      mongo_db.asset_market_info.find({}, {'_id': 0}).sort('market_cap_in_{}'.format(config.XCP.lower()),
                                                           pymongo.DESCENDING).limit(limit))
    assets_market_info_btc = list(
      mongo_db.asset_market_info.find({}, {'_id': 0}).sort('market_cap_in_{}'.format(config.BTC.lower()),
                                                           pymongo.DESCENDING).limit(limit))
    assets_market_info = {
    ...
    assets = list(set([a['asset'] for a in assets_market_info[config.XCP.lower()]] + [a['asset'] for a in
                                                                                      assets_market_info[
                                                                                        config.BTC.lower()]]))
    extended_asset_info = mongo_db.asset_extended_info.find({'asset': {'$in': assets}})
    extended_asset_info_dict = {}
  ...
  def get_market_price_history(asset1, asset2, start_ts=None, end_ts=None, as_dict=False):
    ...
    result = mongo_db.trades.aggregate([
      ...
      {"$sort": SON([("_id.year", pymongo.ASCENDING), ("_id.month", pymongo.ASCENDING), ("_id.day", pymongo.ASCENDING),
    ...
    if as_dict:
      for i in xrange(len(result)):
        result[i]['interval_time'] = int(time.mktime(datetime.datetime(
          result[i]['_id']['year'], result[i]['_id']['month'], result[i]['_id']['day'],
  ...
  def _get_order_book(base_asset, quote_asset,
                      bid_book_min_pct_fee_provided=None, bid_book_min_pct_fee_required=None,
                      bid_book_max_pct_fee_required=None,
                      ask_book_min_pct_fee_provided=None, ask_book_min_pct_fee_required=None,
    ...
    if base_bid_book and base_ask_book:
      ...
      bid_ask_median = float(
        ( D(max(base_ask_book[0]['unit_price'], base_bid_book[0]['unit_price'])) - (D(abs(bid_ask_spread)) / 2) ))
    else:
      bid_ask_spread = 0
      bid_ask_median = 0
  ...
  def get_transaction_stats(start_ts=None, end_ts=None):
    ...
    for e in stats:
      ...
      time_val = int(
        time.mktime(datetime.datetime(e['_id']['year'], e['_id']['month'], e['_id']['day']).timetuple()) * 1000)
  ...
  def get_asset_pair_market_info(asset1=None, asset2=None, limit=50):
    ...
    if asset1 and asset2:
      base_asset, quote_asset = util.assets_to_asset_pair(asset1, asset2)
      pair_info = mongo_db.asset_pair_market_info.find({'base_asset': base_asset, 'quote_asset': quote_asset},
                                                       {'_id': 0})
    else:
      pair_info = mongo_db.asset_pair_market_info.find({}, {'_id': 0}).sort('completed_trades_count',
                                                                            pymongo.DESCENDING).limit(limit)
  ...
  def get_asset_history(asset, reverse=False):
    ...
    * IF type = 'called_back':
    ...
    callbacks = util.call_jsonrpc_api("get_callbacks",
                                      {'filters': {'field': 'asset', 'op': '==', 'value': asset['asset']}},
                                      abort_on_error=True)['result']
    final_history = []
    if len(callbacks):
  ...
  def get_preferences(wallet_id, for_login=False, network=None):
    ...
    if for_login:  # record user login
      ...
      mongo_db.login_history.insert(
        {'wallet_id': wallet_id, 'when': now, 'network': network, 'action': 'login', 'ip': ip, 'ua': ua})
...
def store_preferences(wallet_id, preferences, for_login=False, network=None, referer=None):
  ...
  if for_login:  #mark this as a new signup IF the wallet doesn't exist already
    ...
    if existing_record.count() == 0:
      ...
      mongo_db.login_history.insert({'wallet_id': wallet_id, 'when': now,
                                     'network': network, 'action': 'login', 'ip': ip,
                                     'ua': ua})  #also log a wallet login
...
def proxy_to_counterpartyd(method='', params=[]):
  ...
  if redis_client:  # check for a precached result and send that back instead
    cache_key = method + '||' + base64.b64encode(json.dumps(
      params).encode()).decode()  #^ must use encoding (e.g. base64) since redis doesn't allow spaces in its key names
...
def convert_armory_signedtx_to_raw_hex(signed_tx_ascii):
  ...
  raw_tx_hex = \
  util.call_jsonrpc_api("convert_signed_tx_to_raw_hex", params=params, endpoint=endpoint, abort_on_error=True)['result']
  return raw_tx_hex
...
def handle_get():
  ...
  result = {
    ...
    'counterpartyd_ver': '%s.%s.%s' % (
      cpd_status['version_major'], cpd_status['version_minor'],
      cpd_status['version_revision']) if cpd_result_valid else '?',
...
def handle_post():
  ...
  try:
    ...
    tx_logger.info(
      "TRANSACTION --- %s ||| REQUEST: %s ||| RESPONSE: %s" % (request_data['method'], request_json, rpc_response_json))
  except Exception, e:
...
h = logging_handlers.RotatingFileHandler(os.path.join(config.DATA_DIR, "api.access.log"), 'a', API_MAX_LOG_SIZE,
                                         API_MAX_LOG_COUNT)
